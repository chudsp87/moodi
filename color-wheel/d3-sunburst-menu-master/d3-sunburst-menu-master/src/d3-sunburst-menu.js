...
// export default (function d3_sunburst_menu(tree, node, container) {
    ...
    var backSize = 0.3; // back button size as percent of full circle
    var currentArc, currentNode = tree; // start traversal at root level
    var menuWaiter;
    var idleTime = 0; // time (ms) between edge nudge and traversal
    var padAngle = 0.01;
    ...
    var loaderDuration = 0; // duration of loading arcs in ms
    var menu_level_scope = 3; // number of menu levels to visualise together
    var maxRadius = 190;
    var nudgeTolerance = 0;
    var ratio = .3;
    var fontSizeRatio = 0.4;
    ...
    var arcradius = function(inner_outer) {
        return function(n) {
            // return Math.atan(Math.pow(n.depth, ratio) + Math.pow(inner_outer, ratio) + 1.2 / filler * radius / _radius - 1) * radius - inner_outer * 3 * filler ;
            return (Math.atan(ratio * (n.depth + inner_outer) + 1.2 * radius / _radius - 1)) * radius / (1 - .5*(1-ratio)) - (inner_outer * 3);
        }
    }
    ...
    function traverse(tree, clickLocation) {
            ...
            .each(function() {
                ...
                d3.selectAll("text.curved")
                    ...
                    .attr("font-size", function(d) {
                        return (arc.outerRadius()(d) - arc.innerRadius()(d)) * fontSizeRatio;
                    })
                    .attr("dy", function(d) {
                        return ((arc.outerRadius()(d) - arc.innerRadius()(d)) / 0.7*(1-fontSizeRatio)) * .85;
                    })
                    .transition()
                    .duration(250)
                    .ease("ease-in")
        ...
        d3.select("#"+appName+"-radialmenu").selectAll("g")
            ...
            .attr("dom", function(n){n["dom"] = this;return this});
    ...
    function addCurvedText() {
        ...
        d3.select(this.parentNode)
            ...
            .attr("style", function(n) { if (n.depth < 3) return "font-weight:800;stroke:#000000;stroke-width:1px;stroke-linecap:butt;stroke-linejoin:miter;stroke-opacity:1;"; else return ""});
